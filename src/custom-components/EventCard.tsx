            ...
            <h4>{name}</h4>
                ...
                <div className='flex gap-4 mt-8 '>
                  {events.map((event,index)=>{
                    return <div key={event.id} className={`h-[20px] p-1 w-[20px]  flex justify-center items-center rounded-full ${active === index? 'bg-[var(--primary-yellow)]':" bg-[var(--secondary-yellow)]"} cursor-pointer`} onClick={()=>handleChange(index)}></div>
                  })}
                </div>
            </article>
